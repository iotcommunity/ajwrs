 ...
 * Macro definitions
 **********************************************************************************************************************/
#define GPT_PCLK_TO_COUNTS(x)         (x)
#define GPT_NS_TO_COUNTS(x, freq_hz)  (((x) * ((freq_hz) * 1ULL)) / 1000000000)
#define GPT_US_TO_COUNTS(x, freq_hz)  (((x) * ((freq_hz) * 1ULL)) / 1000000)
#define GPT_MS_TO_COUNTS(x, freq_hz)  (((x) * ((freq_hz) * 1ULL)) / 1000)
#define GPT_S_TO_COUNTS(x, freq_hz)   ((x) * ((freq_hz) * 1ULL))
#define GPT_HZ_TO_COUNTS(x, freq_hz)  (((freq_hz) * 1ULL) / (x))
#define GPT_KHZ_TO_COUNTS(x, freq_hz) (((freq_hz) * 1ULL) / (1000 * (x)))

#define GPT_PERCENT_TO_COUNTS(duty, period) (period * duty / 100ULL)
#define GPT_PERCENT_X_1000_TO_COUNTS(duty, period) (period * duty / 100000ULL)

/** Maximum number of clock counts in 32 bit timer */
#define GPT_MAX_CLOCK_COUNTS (0xFFFFFFFFULL)
...
static ssp_err_t period_to_pclk (timer_size_t const    p_period,
                            ...
                            timer_cfg_t const * const p_cfg)
{
    ...
    {
        err = duty_cycle_to_pclk(p_cfg->duty_cycle, p_cfg->duty_cycle_unit, pclk_counts, &duty_cycle);
        GPT_ERROR_RETURN((SSP_SUCCESS == err), err);
    }
 ...
 * @retval AMSSP_ERR_ASSERTION   The p_ctrl or p_value parameter was null.
 * @retval SSP_ERR_NOT_OPEN      The channel is not opened.
 **********************************************************************************************************************/
                              ...
                              uint8_t          const pin)
{
    ...
    ssp_err_t err = duty_cycle_to_pclk(duty_cycle, unit, HW_GPT_TimerCycleGet(p_ctrl->channel) + 1, &duty_cycle_counts);
    GPT_ERROR_RETURN((SSP_SUCCESS == err), err);
...
ssp_err_t gpt_common_open (timer_ctrl_t * const      p_ctrl, timer_cfg_t const * const p_cfg,
                           gpt_pclk_div_t * const p_pclk_divisor, timer_size_t * const p_pclk_counts)
{
#if GPT_CFG_PARAM_CHECKING_ENABLE
    ...
    ssp_err_t     err          = SSP_SUCCESS;
    err = period_to_pclk(p_cfg->period, p_cfg->unit, p_pclk_counts, p_pclk_divisor);
    if (SSP_SUCCESS != err)
    {
        R_BSP_HardwareUnlock(lock);
 ...
 *                                        - Upper bound: (0xFFFFFFFF * 1024 / (PCLK frequency))
 **********************************************************************************************************************/
static ssp_err_t period_to_pclk (timer_size_t const period, timer_unit_t const unit, timer_size_t * const p_period_pclk,
                                 gpt_pclk_div_t * const p_divisor)
{
    ...
    switch (unit)
    {
        case TIMER_UNIT_PERIOD_RAW_COUNTS:
            temp_period = GPT_PCLK_TO_COUNTS(period);
            break;
        case TIMER_UNIT_FREQUENCY_KHZ:
            temp_period = GPT_KHZ_TO_COUNTS(period, pclk_freq_hz);
            break;
        case TIMER_UNIT_FREQUENCY_HZ:
            temp_period = GPT_HZ_TO_COUNTS(period, pclk_freq_hz);
            break;
        case TIMER_UNIT_PERIOD_NSEC:
            temp_period = GPT_NS_TO_COUNTS(period, pclk_freq_hz);
            break;
        case TIMER_UNIT_PERIOD_USEC:
            temp_period = GPT_US_TO_COUNTS(period, pclk_freq_hz);
            break;
        case TIMER_UNIT_PERIOD_MSEC:
            temp_period = GPT_MS_TO_COUNTS(period, pclk_freq_hz);
            break;
        case TIMER_UNIT_PERIOD_SEC:
            temp_period = GPT_S_TO_COUNTS(period, pclk_freq_hz);
            break;
        default:
    ...
    while ((temp_period > GPT_MAX_CLOCK_COUNTS) && (temp_div < GPT_PCLK_DIV_BY_1024))
    {
        temp_period >>= 2;
        temp_div++;
    }
    ...
    GPT_ERROR_RETURN((temp_period <= 0xFFFFFFFFULL), SSP_ERR_INVALID_ARGUMENT);
    *p_divisor     = temp_div;
...
static ssp_err_t gpt_set_period (uint8_t const ch, timer_size_t const period, timer_unit_t const unit)
{
    ...
    timer_size_t      pclk_counts  = 0;
    gpt_pclk_div_t pclk_divisor = GPT_PCLK_DIV_BY_1;
    err = period_to_pclk(period, unit, &pclk_counts, &pclk_divisor);
    GPT_ERROR_RETURN((SSP_SUCCESS == err), err);
...
static void gpt_set_duty_cycle (uint8_t const ch, timer_size_t const duty_cycle_counts, uint8_t const pin)
{
    ...
    else if (0xFFFFFFFF == duty_cycle_counts)
    {
        ...
        timer_size_t const period, timer_size_t * const p_duty_cycle_pclk)
{
    ...
    uint64_t temp_duty_cycle = 0xFFFFFFFE;
    ...
    switch (unit)
    {
        ...
        case TIMER_PWM_UNIT_PERCENT:
            ...
            {
                temp_duty_cycle = GPT_PERCENT_TO_COUNTS(duty_cycle, period);
            }
            break;
        case TIMER_PWM_UNIT_PERCENT_X_1000:
            ...
            {
                temp_duty_cycle = GPT_PERCENT_X_1000_TO_COUNTS(duty_cycle, period);
            }
            break;
        default:
            GPT_ERROR_RETURN(0, SSP_ERR_INVALID_ARGUMENT);
    ...
    GPT_ERROR_RETURN(((0xFFFFFFFF == temp_duty_cycle) || (temp_duty_cycle <= period)), SSP_ERR_INVALID_ARGUMENT);
                                ...
                                timer_mode_t mode, timer_size_t duty_cycle)
{
    ...
    {
        HW_GPT_CompareMatchSet(ch, gtio, 0);
