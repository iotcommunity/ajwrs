 ...
 * Typedef definitions
 **********************************************************************************************************************/
typedef struct priv_peripheral_hdl_t * peripheral_hdl_t;
...
ssp_err_t R_CGC_SystemClockSet (cgc_clock_t clock_source, cgc_system_clock_cfg_t * p_clock_cfg)
{
    ...
    {   // The requested frequency is <= 32MHz
        HW_CGC_SRAM_RAMWaitSet(CGC_SRAM_ZERO_WAIT_CYCLES);  ///< No RAM wait cycles
    }
#elif BSP_MCU_GROUP_S7G2
      uint8_t  new_rom_wait_state = 0;
      ...
      {
          HW_CGC_SRAM_RAMWaitSet(CGC_SRAM_ZERO_WAIT_CYCLES);
      }
      else
      {
          HW_CGC_SRAM_RAMWaitSet(CGC_SRAM_ONE_WAIT_CYCLES);
      }
      ...
      {
          HW_CGC_SRAM_HSRAMWaitSet(CGC_SRAM_ZERO_WAIT_CYCLES);
      }
      else
      {
          HW_CGC_SRAM_HSRAMWaitSet(CGC_SRAM_ONE_WAIT_CYCLES);
      }

      /** Calculate the Wait states for ROM */
       if (requested_frequency < CGC_SYS_CLOCK_FREQ_ONE_ROM_WAITS)
       {
           new_rom_wait_state = 0;
       }
       else
       if (requested_frequency < CGC_SYS_CLOCK_FREQ_TWO_ROM_WAITS)
       {
          new_rom_wait_state = 1;
       }
       else
       {
           new_rom_wait_state = 2;
       }
    ...
    current_frequency =
        HW_CGC_ClockHzGet(CGC_SYSTEM_CLOCKS_ICLK);
    if ((!(current_frequency > CGC_MAX_ZERO_WAIT_FREQ)) && HW_CGC_HighSpeedMode())
    {
        HW_CGC_MemWaitSet(CGC_ZERO_WAIT_CYCLES);
    }
